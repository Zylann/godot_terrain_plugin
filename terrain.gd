...
func _on_terrain_size_changed():
	...
	if is_inside_tree():
		...
		resize_grid(_data, terrain_size+1, terrain_size+1, 0)
		resize_grid(_normals, terrain_size+1, terrain_size+1, Vector3(0,1,0))
		resize_grid(_chunks, _chunks_x, _chunks_y, funcref(self, "_create_chunk_cb"), funcref(self, "_delete_chunk_cb"))
